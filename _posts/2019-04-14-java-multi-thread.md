...
2. 活跃性问题
串行程序中活跃性问题形式之一：无意中造成无限循环，循环之后的代码无法得到执行。

多线程带来的其他问题：死锁、活锁、饥饿等等

3. 性能问题
性能问题与活跃性问题密切相关，包含多个方面，如：服务时间过长、响应不灵敏、吞吐率过低、资源消耗高、可伸缩性较低等等。

良好的并发程序设计能提高程序性能，但线程总会带来某种程度的运行时开销，比如：
...
每个Servlet都表示一个程序逻辑组件，在高吞吐率的网站中，多个客户端可能会同时请求同一个Servlet服务。在Servlet规范中，Servlet同样需要满足被多个线程同时调用（servlet必须是线程安全的）。
Servlet和JSP，以及ServletContext和httpsession等容器中保存的Servlet过滤器和对象等，都必须是线程安全的。
