...
## 权限共享
**定义:** 设G0是一个保护图，x,y是$$ G_0 $$中两个不同实体节点，  $$ \alpha $$  是权限集合$$ R $$的一个子集。谓词$$ can·share(\alpha,x,y，G_0) $$的意义是：
...
## 共享模型
**定理：** 谓词$$ can·share(\alpha,x,y，G_0) $$的意义是：
...
**定理：** 谓词$$ can·share(\alpha,x,y，G0) $$为真的充要条件是：
...
## 偷窃模型(steal mode)
**定义：** 设G0是一个保护图，x,y是G0中两个不同实体节点，α是权限集合R的一个子集。谓$$ can·steal(α,x,y) $$的意义是：
